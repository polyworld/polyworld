...
float critter::Update(float moveFitnessParam, float speed2dpos, int solidObjects)
{
#ifdef DEBUGCHECK
...
//	printf( "%g = %g\n", populationEnergyPenalty, denergy );
	...
	{
		// If the critter moves, then we want to do collision avoidance
		if( dx != 0.0 || dz != 0.0 )
		{
...
void critter::AvoidCollisions( int solidObjects )
{
...
void critter::AvoidCollisionDirectional( int direction, int solidObjects )
{
	...
	float crtRadius = radius() * CollisionRadiusReductionFactor;
	...
	while( objectxsortedlist::gXSortedObjects.anotherObj( direction, solidObjects, &obj ) )
	{
		...
		if( direction == NEXT )
		{
			if( obj->x() - objRadius > max( x(), LastX() ) + crtRadius )
				break;
		}
		else	// direction == PREV
		{
			if( obj->x() + objRadius < min( x(), LastX() ) - crtRadius)
				break;
		}
		
		// Test to see if we're too far away in z; if so, we're done with this object
		if( obj->z() - objRadius > max( z(), LastZ() ) + crtRadius  ||
			obj->z() + objRadius < min( z(), LastZ() ) - crtRadius )
			continue;
		...
		if( dssquared < dosquared )
		{
			...
			GetCollisionFixedCoordinates( LastX(), LastZ(), x(), z(), obj->x(), obj->z(), crtRadius, objRadius, &xf, &zf );
			setx( xf );
			setz( zf );
